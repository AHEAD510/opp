...
function initNewsSwiper(){
  ...
  if(newsEl.swiper){
    try { newsEl.swiper.destroy(true, true); } catch (e) {}
  }
  if(newsEl.dataset) delete newsEl.dataset.swiperInited;
  newsEl.classList.remove("is-swiper-initialized");

  if(!newsEl.querySelector(".swiper-slide")) return;
  ...
  new Swiper(newsEl,{
    ...
    on: {
      init: function () {
        newsEl.classList.add("is-swiper-initialized");
      }
    }
  });
}
